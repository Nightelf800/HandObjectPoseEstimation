...
eval_order = "--path MSRA_P0/checkpoints --save_preds best --pred_file_name preds_0.txt"
...
###############################################3
@torch.no_grad()
def evaluate(model_path, model_name, loader, args, test_set, OutputToPred, print_detail_crop=True,
             print_detail_uvd=False, print_detail_xyz=False):
    ...
    for i, data in enumerate(loop):
        ...
        inputs, gt2Dcrop, gt2Dorignal, gt3Dorignal, com, M_inv, cubesize = data[0].to(device), data[1].to(device), data[
            2].to(device), data[3].to(device), data[4].to(device), data[5].to(device), data[6].to(device)
    ...
    save_xyz_eval(estimation_xyz, res_path, "json")
    ...
    if args.save_results:
        res_detail_path = os.path.join(res_path, "results.txt")
        f = open(res_detail_path, "a+")
...
# Save xyz_eval
def save_xyz_eval(res, res_path, type="json"):
    file_res_path = os.path.join(res_path, "res3." + type)

    if type == "json":
        res_xyz_list = dict()
        res_xyz_list["model_name"] = "hglass_2_1"
        res_xyz_list["predict"] = [roundall(x.tolist()) for x in res]
        send_hand_joint_to_unity()

        with open(file_res_path, "w") as fo:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--center_refined', default=1, type=int,
                        help="determine if the the results are written into a file")
    parser.add_argument('--dataset', default="nyu", choices=('nyu', 'icvl', 'msra'), type=str,
                        help="which dataset to use")
    parser.add_argument('--save_preds', default="none", choices=('none', 'best', 'last'), type=str,
    ...
    parser.add_argument('--num_clean', default=1, type=int,
    ...
    if args.dataset == "nyu":
        ...
        if os.environ.get('NYU_PATH') is None:
            os.environ["NYU_PATH"] = args.datasetpath
        ...
        if os.environ.get('ICVL_PATH') is None:
            os.environ["ICVL_PATH"] = args.datasetpath
        test_set = ICVLHandPoseDataset(train=False, basepath=os.environ.get('ICVL_PATH'),
                                       center_refined=args.center_refined)
    ...
    final_3Derror, prediction_UVDorig = evaluate(model_path, file, testloader, args, test_set, eval_function,
                                                 print_detail_crop=True, print_detail_uvd=True,
                                                 print_detail_xyz=True)
